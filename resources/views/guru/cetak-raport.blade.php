                    ...
                    <tbody>
                        @foreach($data as $key => $siswa)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('lihat-cetak-raport', ['nisn' => $siswa->NISN, 'id_kelas' => $siswa->id_kelas, 'semester' => request('semester', 0)]) }}" class="text-dark btn btn-success px-4 data-nilai-link" data-nisn="{{ $siswa->NISN }}" data-id-kelas="{{ $siswa->id_kelas }}">Lihat Raport</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<script>
    document.getElementById('semester_dropdown').addEventListener('change', function() {
        var semester = this.value; // Get the selected semester value
        ...
        links.forEach(function(link) {
            ...
            var url = "{{ route('lihat-cetak-raport', ['nisn' => ':nisn', 'id_kelas' => ':id_kelas', 'semester' => ':semester']) }}"
                        .replace(':nisn', nisn)
    ...
    document.getElementById('semester_dropdown').dispatchEvent(new Event('change'));
</script>
@endsection
