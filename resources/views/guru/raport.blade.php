...
<div class="container pb-4 mb-4">
    <div class="card">
        ...
        <div class="container">
            ...
            <form class="form-inline mb-2" action="{{ route('import-kompetensi') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <button type="submit" class="btn btn-primary">Import Kompetensi</button>
            </form>
        </div>
                    ...
                    <tbody>
                        @foreach($data as $key => $siswa)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('siswa-kompetensi', ['nisn' => $siswa->NISN, 'id_kelas' => $siswa->id_kelas, 'semester' => '0']) }}" class="text-dark btn btn-success px-4 data-nilai-link" data-nisn="{{ $siswa->NISN }}" data-id-kelas="{{ $siswa->id_kelas }}">Lihat Kompetensi</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
...
<script>
    document.getElementById('semester_dropdown').addEventListener('change', function() {
        ...
        links.forEach(function(link) {
            ...
            var url = "{{ route('siswa-kompetensi', ['nisn' => ':nisn', 'id_kelas' => ':id_kelas', 'semester' => ':semester']) }}"
                        .replace(':nisn', nisn)
